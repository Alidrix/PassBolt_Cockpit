  ...
  $('logsAuditView').innerHTML = `
    ...
    <div class="card sticky-filters logs-filters-card">
      <div class="form-grid">
        ...
        <div><label>Scope</label><select id="logsScopeFilter"><option value="">Tous</option><option value="import">Import</option><option value="delete">Delete</option><option value="system">System</option><option value="groups">Groups</option></select></div>
        ...
        <div><label>Recherche texte</label><input id="logsTextSearch" placeholder="email, message, path..." /></div>
      </div>
      <div class="action-bar mt-3">
        ...
        <button id="logsDeleteBatchBtn" class="btn btn-secondary">Supprimer logs batch</button>
        <button id="logsDeleteAllBtn" class="btn btn-danger">Supprimer tous les logs</button>
      </div>
    </div>

    <div class="card logs-audit-card">
      <div class="tabs">
        <button class="btn btn-secondary tab-btn active" data-tab="summaryTab">Synthèse</button>
        <button class="btn btn-secondary tab-btn" data-tab="streamTab">Flux technique</button>
        <button class="btn btn-secondary tab-btn" data-tab="tableTab">Tableau détaillé</button>
        <button class="btn btn-secondary tab-btn" data-tab="auditTab">Audit métier</button>
      </div>
...
function renderSummary(summary = {}, rows = []) {
  $('summaryTab').innerHTML = `
    <div class="grid-kpi">
      ...
      ${kpiCard('Audit', summary.by_level?.audit || 0)}
      ${kpiCard('Dernier log', summary.last_log?.message || 'Aucun')}
    </div>
    ${rows.length ? '' : `<div class="mt-3">${emptyState('Aucun log disponible pour les filtres actuels.')}</div>`}
  `;
}
...
export async function refreshLogs() {
  ...
  if (logsError) {
    ...
    setToast(`Logs indisponibles: ${logsError}`, 'error');
    return;
  }
...
async function deleteLogs(byBatch = false) {
  try {
    ...
    if (byBatch) {
      ...
      if (!batch) return setToast('Sélectionnez un batch.', 'warning');
      url += `?batch_uuid=${encodeURIComponent(batch)}`;
    }
    await apiDelete(url);
    refreshLogs();
    setToast('Suppression effectuée.', 'success');
